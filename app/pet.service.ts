...
export class PetService {
  ...
  findPets(animal : string) {
    ...
    // Return response
   return this.jsonp.get(this.petsUrl + endPoint, { search: params }).map(response => 
   <Pet[]> response.json().petfinder.pets.pet);
  }
  ...
  findPetById(id: string){
    ...
    console.log(id);
    // Return response
   return this.jsonp .get(this.petsUrl + endPoint, { search: params })
              .map(response => {
